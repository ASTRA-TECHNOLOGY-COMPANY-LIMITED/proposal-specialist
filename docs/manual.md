...
```
[기업 문서] → analyze-company → search-bid → search-evaluate → 평가 보고서
                ↕           ↕          ↕
            기업 프로필    공고 목록    SWOT + 전략
```

### Step 1: 기업 문서 분석 (`analyze-company`)

```
/proposal-specialist:analyze-company ./회사소개서.pdf
```
...
### Step 2: 공고 검색 (`search-bid`)

```
/proposal-specialist:search-bid AI 솔루션 클라우드
```
...
### 원스텝: 전체 워크플로우 (`search-strategy`)

```
/proposal-specialist:search-strategy ./회사소개서.pdf
```
...
[RFP 문서] + [기업 시드 문서]
    ...
    → generate-section → sections/*.html (완전한 HTML 페이지)
```
...
# 특정 섹션만 작성
/proposal-specialist:generate-section data/output/사업명/목차.md 1 3 5
```
...
특정 섹션을 재작성하려면:
```
/proposal-specialist:generate-section data/output/사업명/목차.md 3
```
...
**Q: 특정 섹션만 재작성할 수 있나요?**
A: `generate-section` 명령에 섹션 번호를 지정하면 해당 섹션만 재작성합니다.

**Q: API 키 없이 사용할 수 있나요?**
A: `DATA_GO_KR_API_KEY`는 필수입니다. 이 키 없이는 공고 검색이 불가합니다. 제안서 작성(generate-toc, generate-section)은 API 키 없이도 사용 가능합니다.
